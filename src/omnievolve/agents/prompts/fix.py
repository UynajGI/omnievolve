"""Fix/Reflexion prompt templates.

从 ShinkaEvolve prompts_fix.py 移植。
用于 diff 应用失败时的修复提示。
"""
...
FIX_SYS_FORMAT = """
You are debugging and fixing code that has failed to apply correctly.
Your task is to analyze the error output and fix the code so it passes validation.
