-- OmniEvolve v0.2 SQLite Schema
-- S1-10: Candidate/Lineage/Evaluation/Policy 等 v0.2 表
-- S1-11: job lease、outbox、prompt、memory、telemetry 表
...
CREATE INDEX IF NOT EXISTS idx_experiment_task
    ON experiment(task_id);
...
CREATE INDEX IF NOT EXISTS idx_thought_experiment
    ON thought_record(experiment_id);
...
CREATE INDEX IF NOT EXISTS idx_candidate_status
    ON candidate(status);
CREATE INDEX IF NOT EXISTS idx_candidate_island
    ON candidate(island_id);
...
CREATE INDEX IF NOT EXISTS idx_policy_status
    ON search_policy_version(status);
...
CREATE INDEX IF NOT EXISTS idx_meta_eval_policy
    ON meta_evaluation_window(search_policy_id);
...
CREATE INDEX IF NOT EXISTS idx_vector_job_status
    ON vector_index_job(status);
...
CREATE INDEX IF NOT EXISTS idx_job_status
    ON job(status);
CREATE INDEX IF NOT EXISTS idx_job_lease
...
CREATE TABLE IF NOT EXISTS llm_call_ledger (
    id                  TEXT PRIMARY KEY,
    experiment_id       TEXT REFERENCES experiment(id),
    agent_role          TEXT NOT NULL,              -- director/coder/critic/meta
    model               TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_llm_call_experiment
    ON llm_call_ledger(experiment_id);
CREATE INDEX IF NOT EXISTS idx_llm_call_role
    ON llm_call_ledger(agent_role);

-- ═══════════════════════════════════════════
-- FTS5 全文检索（条件创建）
-- ═══════════════════════════════════════════
-- 注意：FTS5 表在应用层检测能力后创建
-- CREATE VIRTUAL TABLE IF NOT EXISTS thought_fts USING fts5(
--     thought_id,
--     content,
--     mechanism_tags,
--     content='thought_record',
--     content_rowid='rowid'
-- );
--
